#!/usr/bin/env bash
# Contratto Articoli: Prezzo di Listino + Prezzo Codice (script autocontenuto, no curl nested).
# Marker: quote-item-catalog-prices-inline-20260606
set -euo pipefail

CRM_ROOT="${CRM_ROOT:-$HOME/public_html/crm/mec-group}"
cd "${CRM_ROOT}" || { echo "ERRORE: cartella CRM non trovata: ${CRM_ROOT}" >&2; exit 1; }
...
write_file() {
  ...
  tmp="$(mktemp "${TMPDIR:-/tmp}/espo-inline.XXXXXX")"
  cat > "${tmp}"
  mv "${tmp}" "${dest}"
  echo "OK ${dest}"
}

echo "=== Backup (quote-item-catalog-prices-inline-20260606) ==="
cp -a "custom/Espo/Custom/Services/QuotePricingCalculator.php" "${BK}/QuotePricingCalculator.php" 2>/dev/null || true
cp -a "custom/Espo/Custom/Tools/Quote/Api/PostGetItemCatalogPrices.php" "${BK}/PostGetItemCatalogPrices.php" 2>/dev/null || true
cp -a "custom/Espo/Custom/Resources/routes.json" "${BK}/routes.json" 2>/dev/null || true
cp -a "custom/Espo/Custom/Resources/client/custom/src/views/quote/fields/item-list.js" "${BK}/item-list.js" 2>/dev/null || true
cp -a "custom/Espo/Custom/Resources/client/custom/src/views/quote/record/item.js" "${BK}/item.js" 2>/dev/null || true
cp -a "client/custom/src/views/quote/fields/item-list.js" "${BK}/item-list.js" 2>/dev/null || true
cp -a "client/custom/src/views/quote/record/item.js" "${BK}/item.js" 2>/dev/null || true
echo "Backup: ${BK}/"

echo ""
echo "=== Scrivo file (quote-item-catalog-prices-inline-20260606) ==="

write_file "custom/Espo/Custom/Services/QuotePricingCalculator.php" <<'EOF_custom_Espo_Custom_Services_QuotePricingCalculator_php'
<?php
 ...
 * - vendita abbinata: se attiva, Importo Codice Totale scontato di scontoVenditaAbbinata %
 *
...
class QuotePricingCalculator
{
    ...
    private function syncTotalsAndDerivedFields(Entity $entity, bool $isQuote): void
    {
        ...
        if ($isQuote) {
            $this->applyVenditaAbbinataScontoPrezzoCodice($entity);
        }
        ...
        if ($prezzoListino !== null && $prezzoListino > 0) {
            ...
            $codiceNetTot = $this->floatOrNull($entity->get('prezzoCodiceIvaEsclusa'));

            if ($codiceNetTot !== null && $codiceNetTot > 0) {
                $entity->set(
                    'margineSuListino',
                    round((($imponibile - $codiceNetTot) / $codiceNetTot) * 100, 2)
                );
    ...
    /**
     * Vendita abbinata: sconto % su Importo Codice Totale (IVI) e prezzo codice netto.
     */
    private function applyVenditaAbbinataScontoPrezzoCodice(Entity $quote): void
    {
        if (!$quote->get('venditaAbbinataAdAltroProdotto')) {
            return;
        }

        $sconto = $this->floatOrNull($quote->get('scontoVenditaAbbinata'));
        ...
        foreach (['totalPrezzoCodice', 'prezzoCodiceIvaEsclusa', 'prezzoCodiceIvaInclusa'] as $field) {
            if (!$quote->hasAttribute($field)) {
                continue;
            }
            ...
            if ($value !== null && $value > 0) {
                $patch[$field] = round($value * $factor, 2);
            }
        }
...
EOF_custom_Espo_Custom_Services_QuotePricingCalculator_php

write_file "custom/Espo/Custom/Tools/Quote/Api/PostGetItemCatalogPrices.php" <<'EOF_custom_Espo_Custom_Tools_Quote_Api_PostGetItemCatalogPrices_php'
<?php
...
EOF_custom_Espo_Custom_Tools_Quote_Api_PostGetItemCatalogPrices_php

write_file "custom/Espo/Custom/Resources/routes.json" <<'EOF_custom_Espo_Custom_Resources_routes_json'
[
    {
        "route": "Quote/getItemCatalogPrices",
        "method": "post",
        "actionClassName": "Espo\\Custom\\Tools\\Quote\\Api\\PostGetItemCatalogPrices"
    }
]
EOF_custom_Espo_Custom_Resources_routes_json

write_file "custom/Espo/Custom/Resources/client/custom/src/views/quote/fields/item-list.js" <<'EOF_custom_Espo_Custom_Resources_client_custom_src_views_quote_fields_item_list_js'
// ========================================
// VERSIONE: 1.2.0
// DATA: 2026-06-06
...
EOF_custom_Espo_Custom_Resources_client_custom_src_views_quote_fields_item_list_js

write_file "custom/Espo/Custom/Resources/client/custom/src/views/quote/record/item.js" <<'EOF_custom_Espo_Custom_Resources_client_custom_src_views_quote_record_item_js'
// ========================================
// VERSIONE: 1.0.0
// DATA: 2026-06-06
...
EOF_custom_Espo_Custom_Resources_client_custom_src_views_quote_record_item_js

write_file "client/custom/src/views/quote/fields/item-list.js" <<'EOF_client_custom_src_views_quote_fields_item_list_js'
// ========================================
// VERSIONE: 1.2.0
// DATA: 2026-06-06
// FILE: custom/Espo/Custom/Resources/client/custom/src/views/quote/fields/item-list.js
...
EOF_client_custom_src_views_quote_fields_item_list_js

write_file "client/custom/src/views/quote/record/item.js" <<'EOF_client_custom_src_views_quote_record_item_js'
// ========================================
// VERSIONE: 1.0.0
// DATA: 2026-06-06
// Popola Prezzo di Listino e Prezzo Codice alla selezione prodotto.
...
EOF_client_custom_src_views_quote_record_item_js
...
echo ""
echo "Deploy completato (quote-item-catalog-prices-inline-20260606)."
echo "Test: contratto → listino + prodotto → Prezzo di Listino e Prezzo Codice."
echo "Rollback: ripristina da ${BK}/ e php command.php rebuild"
