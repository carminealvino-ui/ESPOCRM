#!/usr/bin/env bash
# Deploy Contratti completo: metadata PR #90 + provvigioni imponibile netto PR #99.
# NON tocca client.json / CSS KPI (usare deploy-kpi-layout-lordi-pianificati.sh).
#
# Uso:
...
BRANCH="${BRANCH:-cursor/fix-contratto-stato-provvigioni-9999}"
...
echo "=== Branch: ${BRANCH} (PR #90 layout + PR #99 provvigioni) ==="
...
remove_double_crea_prodotto() {
  echo ""
  echo "=== Rimuove pulsante Crea prodotto duplicato (PR #9) ==="
  ...
  local cj="custom/Espo/Custom/Resources/metadata/app/client.json"
  if [[ -f "${cj}" ]] && grep -q 'custom-product-button' "${cj}" 2>/dev/null; then
    ...
    ' "${cj}"
    echo "OK rimosso custom-product-button da ${cj}"
  fi
}
...
if [[ "${SKIP_BACKUP}" != "1" ]]; then
  ...
  echo "=== Backup ==="
  TS="$(date +%Y%m%d-%H%M%S)"
  BK="custom/backup-layouts/contratto-completo-${TS}"
  mkdir -p "${BK}"
...
php tools/verify-contratto-quote-deploy.php || {
  echo "ERRORE: verifica deploy fallita"
  exit 1
}
...
echo ""
echo "=== Fatto: Contratti completo ==="
echo "Atteso:"
echo "  - Colonne Stato Contratto / Stato Finanziamento valorizzate"
echo "  - Pannelli: Articoli → Finanziamento → Provvigioni"
echo "  - Un solo pulsante Crea prodotto (menu …)"
echo "  - Provvigioni su imponibile IVA esclusa; totale = somma importo consolidato"
echo ""
echo "KPI layout (separato):"
echo "  curl -fsSL \"https://raw.githubusercontent.com/carminealvino-ui/ESPOCRM/${BRANCH}/tools/deploy-kpi-layout-lordi-pianificati.sh\" | bash"
