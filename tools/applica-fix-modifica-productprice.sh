#!/usr/bin/env bash
# Fix errore Modifica ProductPrice: Unknown column priceBook.tax_code_name
#   cd ~/public_html/crm/mec-group && curl -fsSL ".../applica-fix-modifica-productprice.sh" | bash
set -euo pipefail
CRM_ROOT="${CRM_ROOT:-$HOME/public_html/crm/mec-group}"
cd "${CRM_ROOT}" || exit 1
mkdir -p tools
cat > tools/fix-pricebook-tax-code-name-column.php << 'EOFPHP'
<?php
...
rm -rf data/cache/*
echo "Fatto. Riprova Modifica sulla riga Prezzi."
