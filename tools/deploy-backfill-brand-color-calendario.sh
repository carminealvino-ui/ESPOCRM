#!/usr/bin/env bash
# Deploy rapido: script backfill brand/colore calendario + servizio PHP + metadata.
#
...
echo ""
echo "=== Rebuild (campo color su ProductBrand, link brand su Disponibilità) ==="
...
echo "Fatto. Prova:"
echo "  cd ${CRM_ROOT}"
echo "  php tools/backfill-brand-color-calendario.php --dry-run --verbose"
echo "  cp tools/data/brand-calendar-colors.example.json tools/data/brand-calendar-colors.json"
echo "  php tools/backfill-brand-color-calendario.php --apply-default-colors --verbose"
echo "  php tools/backfill-brand-color-calendario.php --force-color"
