...
### Status dopo il push (cache)

Se `status` mostra ancora migliaia di **Solo prod** subito dopo un push riuscito, la cartella  
`exports/sync/.cache/repo-main` è quasi sempre **vecchia** (snapshot prima del push).

```bash
rm -rf exports/sync/.cache/repo-main
...
```bash
php tools/sync-custom-prod-repo.php status --local-repo=exports/sync/github-push-20260601-084656/repo
```
