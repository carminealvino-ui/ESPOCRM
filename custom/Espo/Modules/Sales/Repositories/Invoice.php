...
/**
 * @extends Database<InvoiceEntity>
 */
class Invoice extends Database implements Di\InjectableFactoryAware
{
    ...
    {
        $helper = $this->getStatusLockingHelper();

        $applyIssuanceCheck = $helper->toApplyCheck($entity);

        if (
            $entity->isNew() &&
            $applyIssuanceCheck &&
            !empty($options[CoreSaveOption::IMPORT]) &&
            !$helper->isPreIssued($entity)
        ) {
            $entity->setStatus(InvoiceEntity::STATUS_DRAFT);
        }
        ...
        if (
            ...
            !$entity->isRoundingProfileChanged() &&
            !$entity->isItemListChanged() &&
            !$entity->isLocalCurrencyChanged() &&
            !$entity->isCurrencyRateChanged() &&
            !$entity->isPaymentTermsProfileChanged() &&
            !$entity->isDateInvoicedChanged() &&
            !$entity->isDateDueChanged() &&
