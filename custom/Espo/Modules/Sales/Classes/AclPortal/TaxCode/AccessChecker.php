...
namespace Espo\Modules\Sales\Classes\AclPortal\TaxCode;
...
/**
 * @implements AccessEntityCREDChecker<TaxCode>
 */
class AccessChecker implements AccessEntityCREDChecker
{
    ...
    {
        if ($this->aclManager->checkScope($user, Tax::ENTITY_TYPE, Table::ACTION_EDIT)) {
            return true;
        }

        foreach (OrderEntityUtil::getEntityTypesWithTax() as $itEntityType) {
            if (
                $this->aclManager->checkScope($user, $itEntityType, Table::ACTION_CREATE) ||
                $this->aclManager->checkScope($user, $itEntityType, Table::ACTION_EDIT)
            ) {
                return true;
            }
        }
