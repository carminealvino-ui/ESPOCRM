...
/**
 * @implements LinkChecker<Invoice, Tax>
 */
class TaxChecker implements LinkChecker
{
    ...
    public function check(User $user, Entity $entity, Entity $foreignEntity): bool
    {
        ...
        if ($entity->getPrecedingInvoice()?->getTax()?->getId() === $foreignEntity->getId()) {
            return true;
        }

        if ($entity->getSalesOrder()?->getTax()?->getId() === $foreignEntity->getId()) {
            return true;
        }

        if ($entity->getQuote()?->getTax()?->getId() === $foreignEntity->getId()) {
            return true;
        }
