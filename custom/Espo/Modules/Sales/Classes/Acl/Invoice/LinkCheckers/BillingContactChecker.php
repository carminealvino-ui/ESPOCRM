 ...
 */
class BillingContactChecker implements LinkChecker
{
    ...
    public function check(User $user, Entity $entity, Entity $foreignEntity): bool
    {
        ...
        if ($entity->getPrecedingInvoice()?->getBillingContact()?->getId() === $foreignEntity->getId()) {
            return true;
        }

        if ($entity->getSalesOrder()?->getBillingContact()?->getId() === $foreignEntity->getId()) {
            return true;
        }

        if ($entity->getQuote()?->getBillingContact()?->getId() === $foreignEntity->getId()) {
            return true;
        }
