...
/**
 * @implements LinkChecker<Invoice, Quote>
 */
class QuoteChecker implements LinkChecker
{
    ...
    public function check(User $user, Entity $entity, Entity $foreignEntity): bool
    {
        ...
        if ($entity->getPrecedingInvoice()?->getQuote()?->getId() === $foreignEntity->getId()) {
            return true;
        }

        if ($entity->getSalesOrder()?->getQuote()?->getId() === $foreignEntity->getId()) {
            return true;
        }
