...
namespace Espo\Modules\Sales\Hooks\TaxItemRule;
...
/**
 * @implements BeforeSave<TaxItemRule>
 * @implements AfterRemove<TaxItemRule>
 */
class Order implements BeforeSave, AfterRemove
{
    ...
    public function beforeSave(Entity $entity, SaveOptions $options): void
    {
        ...
        $query = SelectBuilder::create()
            ...
            ->group('id')
            ->limit(0, 1)
            ->where(['taxId' => $entity->getTax()->getId()])
    ...
    {
        $taxId = $entity->get(TaxItemRule::ATTR_TAX_ID);

        if (!$taxId) {
            return;
        }

        $this->moveService->reOrder($taxId);
    }
}
