 ...
 * @implements AfterRemove<TaxCode>
 */
class Order implements BeforeSave, AfterRemove
{
    ...
    public function beforeSave(Entity $entity, SaveOptions $options): void
    {
        ...
        $query = SelectBuilder::create()
            ...
            ->select(Expr::max(Expr::column(TaxCode::ATTR_ORDER)), 'max')
            ->select('id')
            ->group('id')
            ->limit(0, 1)
            ->order(Expr::max(Expr::column(TaxCode::ATTR_ORDER)), OrderPart::DESC)
            ->build();
        ...
        $entity->set(TaxCode::ATTR_ORDER, $order);
    }
