...
namespace Espo\Modules\Sales\Tools\TaxPurchaseRule;
...
class RuleService
{
    ...
    public function get(Account $account): ?Tax
    {
        $rules = $this->entityManager
            ->getRDBRepositoryByClass(TaxPurchaseRule::class)
            ->where([
                'status' => TaxPurchaseRule::STATUS_ACTIVE,
            ])
            ->order('order')
            ->limit(0, self::LIMIT)
            ->sth()
    ...
    private function checkRule(ConditionChecker $checker, TaxPurchaseRule $rule): bool
    {
