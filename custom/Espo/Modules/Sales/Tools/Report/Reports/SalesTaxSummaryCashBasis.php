...
class SalesTaxSummaryCashBasis implements GridReport
{
    ...
    public function runSubReport(SearchParams $searchParams, SubReportParams $subReportParams, ?User $user): ListResult
    {
        ...
        $type = match ($subReportParams->getTarget()) {
            self::SWITCH_INVOICES => Type::Inbound->value,
            self::SWITCH_CREDIT_NOTES => Type::Outbound->value,
        };
        ...
        $query = $queryBuilder
            ...
            ->where([
                $link . '.' . PaymentEntry::FIELD_PARTY_TYPE => PartyType::Customer->value,
    ...
    private function prepareSubQuery(?Item $where, string $currency, string $entityType): Select
    {
        ...
        $type = match ($entityType) {
            Invoice::ENTITY_TYPE => Type::Inbound->value,
            CreditNote::ENTITY_TYPE => Type::Outbound->value,
        };
