...
class CustomerQueryHelper
{
    ...
    public function prepareSubQuery(
        ...
        $amountExpr = $this->prepareAmountExpr($entityType, $sourceType, $isLoss);
        ...
        if ($entityType === PaymentEntry::ENTITY_TYPE) {
            ...
            } else if ($sourceType === WriteOffEntry::ENTITY_TYPE) {
                $queryBuilder->leftJoin(PaymentAllocation::LINK_WRITE_OFF);
            } else if ($sourceType === CreditNote::ENTITY_TYPE) {
                $queryBuilder->leftJoin(PaymentAllocation::LINK_CREDIT_NOTE);
            }
        }
    ...
    private function prepareAmountExpr(string $entityType, ?string $sourceType, bool $isLoss = false): Expr
    {
        ...
        if ($entityType === PaymentEntry::ENTITY_TYPE) {
            ...
            if ($sourceType === WriteOffEntry::ENTITY_TYPE) {
                $amountAttribute = PaymentAllocation::FIELD_AMOUNT_LOCAL;
            }
        }
    ...
    {
        $column = OrderEntity::ATTR_ACCOUNT_ID;

        if ($entityType === PaymentAllocation::ENTITY_TYPE) {
            $column = match ($sourceType) {
                PaymentEntry::ENTITY_TYPE => PaymentAllocation::LINK_PAYMENT_ENTRY . '.' . OrderEntity::ATTR_ACCOUNT_ID,
                CreditNote::ENTITY_TYPE => PaymentAllocation::LINK_CREDIT_NOTE . '.' . OrderEntity::ATTR_ACCOUNT_ID,
                WriteOffEntry::ENTITY_TYPE => PaymentAllocation::LINK_WRITE_OFF . '.' . OrderEntity::ATTR_ACCOUNT_ID,
    ...
    private function convertWhereForAllocation(?string $sourceType, Item $where): Item
    {
        ...
        return $this->helper->changeWhereItem(
            item: $where,
            from: OrderEntity::ATTR_ACCOUNT_ID,
            to: $to,
        );
    }
