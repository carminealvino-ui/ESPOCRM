...
class CreditNoteRecordService
{
    ...
    public function find(Subscription $subscription, SearchParams $searchParams): Collection
    {
            ...
            ->where(
                Cond::in(
                    ...
                    SelectBuilder::create()
                        ->select('m.creditNoteId')
                        ->from('CreditNoteSubscriptionUpdate', 'm')
                        ->join(
                            Join
        ...
        $builder = $this->entityManager->getRDBRepositoryByClass(CreditNote::class)->clone($query);
