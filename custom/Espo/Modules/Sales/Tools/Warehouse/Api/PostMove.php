...
class PostMove implements Action
{
    ...
    {
        $warehouse = $this->getRecord($request);
        $type = $this->fetchType($request);
        $searchParams = $this->fetchSearchParams($request);

        $this->service->move($warehouse, $type, $searchParams);
     ...
     * @throws NotFound
     */
    private function getRecord(Request $request): Warehouse
    {
        $id = $request->getRouteParam('id') ?? throw new BadRequest();

        $rule = $this->entityProvider->getByClass(Warehouse::class, $id);
