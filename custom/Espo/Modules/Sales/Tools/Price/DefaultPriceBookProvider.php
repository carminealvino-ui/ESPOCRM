...
class DefaultPriceBookProvider
{
    private ?PriceBook $entity = null;
    ...
    public function get(): ?PriceBook
    {
        ...
        if (!$this->isCached) {
            $priceBook = $this->entityManager->getRDBRepositoryByClass(PriceBook::class)->getById($id);

            $this->isCached = true;

            if ($priceBook->isActive()) {
                $this->entity = $priceBook;
            }
        }
