...
namespace Espo\Modules\Sales\Tools\TaxCode\Api;
...
class PostMove implements Action
{
    ...
    {
        $taxCode = $this->getTaxCode($request);
        $type = $this->fetchType($request);
        $searchParams = $this->fetchSearchParams($request);

        $this->service->move($taxCode, $type, $searchParams);
     ...
     * @throws NotFound
     */
    private function getTaxCode(Request $request): TaxCode
    {
        $id = $request->getRouteParam('id') ?? throw new BadRequest();

        $rule = $this->entityProvider->getByClass(TaxCode::class, $id);
