...
define('outlook:views/outlook/panels/outlook-calendar', ['view'], function (Dep) {
    ...
    return Dep.extend({
        ...
        checkCalendars: function () {
            ...
            for (const key of monitoredCalendars) {
            ...
            if (defaultEntityView.$el.is(':visible')) {
                ...
                for (const key in entities) {
                    ...
                    if ((label == null || label === '') && defaultEntity !== entities[key]) {
                        const msg = this.translate('emptyNotDefaultEnitityLabel', 'messages','OutlookCalendar');
                ...
                if (!defaultIsInList) {
                    const msg = this.translate('defaultEntityIsRequiredInList', 'messages','OutlookCalendar');
                    defaultEntityView.showValidationMessage(msg);
                    notValid |= true;
                }

                if (labelDuplicates) {
                    const msg = this.translate('notUniqueIdentificationLabel', 'messages','OutlookCalendar');
                    enititesView.showValidationMessage(msg, '[data-name="translatedValue"]:last');
                    notValid |= true;
                }
            }
