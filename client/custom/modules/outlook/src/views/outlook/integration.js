...
define('outlook:views/outlook/integration', ['view', 'model'], function (View, Model) {

    return class OutlookIntegrationView extends View {
        ...
        setup() {
            ...
            let skipGraphApi = false;
            let skipTeams = false;

            if (version !== '@@version' && cmp(version, '9.1.0') < 0) {
                skipGraphApi = true;

                this.fieldDataList = this.fieldDataList.filter(it => it.name !== 'graphApiSendEmail')
            }

            if (version !== '@@version' && cmp(version, '9.4.0') < 0) {
                skipTeams = true;

                this.fieldDataList = this.fieldDataList.filter(it => it.name !== 'microsoftTeamsMeetings')
            }

            this.model.setDefs({fields: fieldDefs});
            this.model.populateDefaults();

            this.model.set('redirectUri', this.getConfig().get('siteUrl').replace(/\/$/, '') + '/oauth-callback.php');
            ...
            this.wait(
                (async () => {
                    ...
                    Object.keys(fields).forEach(name => {
                        if (name === 'graphApiSendEmail' && skipGraphApi) {
                            return;
                        }

                        if (name === 'microsoftTeamsMeetings' && skipTeams) {
                            return;
                        }
