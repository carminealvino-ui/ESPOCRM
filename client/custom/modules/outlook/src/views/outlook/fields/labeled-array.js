...
define('outlook:views/outlook/fields/labeled-array', ['views/fields/array'], function (Dep) {
    ...
    return Dep.extend({
        ...
        fetch: function () {
            ...
            for (const key in data[this.name]){
                var scope = data[this.name][key];

                data[scope+ 'IdentificationLabel'] = this.$el
                    .find('.list-group .list-group-item input[data-name="translatedValue"][data-value="' + scope + '"]')
                    .val();
            }
            //hack
            ...
            for (const key in value) {
                ...
                if (label == null || label === '') {
                    if (hasEmptyIdentLabel) {
                        const msg = this.translate('fieldLabelIsRequired', 'messages','OutlookCalendar')
                            .replace('{field}', this.translate(this.name, 'fields', this.model.name));
